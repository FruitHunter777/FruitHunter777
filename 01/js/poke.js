$(function () {
  ...
  for (let i = 0; i < 7; i++) {
    for (let j = 0; j <= i; j++) {
      index++;
      let obj = poke[index];
      let lefts = 130 - 20 * i + 40 * j;
      let tops = 25 * i + 40;
      $("<div>")
        .addClass("poke")
        .css({ backgroundImage: `url(./imgs/${obj.number}${obj.color}.png)` })
        .appendTo(".desk")
  ...
  for (; index < 52; index++) {
    ...
    $("<div>")
      ...
      .data("number", obj.number)
      .css({
        backgroundImage: `url(./imgs/${obj.number}${obj.color}.png)`,
        opacity: 1,
        zIndex: index - start + 1,
      })
      .appendTo(".desk");
  }
